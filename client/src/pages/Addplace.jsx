...
export default function Addplace(){
    ...
    const AddphotoByLink = async (ev) => {
        ev.preventDefault();
       const res = await axios.post('/upload-by-link',{link: photosLink});
    ...
    const HandleUpload = async (ev) => {
      ...
      const addNewplace = async (ev) => {
        ...
        if(title.length === 0 || address.length === 0 || description.length === 0 || extraInfo.length === 0 || checkin.length === 0 || checkout.length === 0 || maxguest.length === 0 || price.length === 0){   
                ...
                <div className="">
                    ...
                    <div className="my-6 grid lg:grid-cols-2 lg:space-x-4">
                        <div className="">
                            ...
                            <input value={photosLink} onChange={ev => setPhotosLink(ev.target.value)} className="rounded-full w-full border-2 px-2 py-2 focus:outline-none border-gray-600" placeholder="Add using a link to your photos....jpg"></input>
                            {!isValid && <div>
                            <i className="text-red-500">Use an http or https Link. (eg:https://149345965.v2.pressablecdn.com)</i>
                            </div>}
                            {!found && <div>
                            <i className="text-red-500">Invalid Link provided.</i>
                            </div>}
                            
                        </div>
                        <div className="flex items-start lg:items-center mt-4 lg:mt-20">
                            <button onClick={AddphotoByLink} className="bg-lightblue px-4 py-2 rounded text-white font-bold">Add photo</button>
                        </div> 
                    </div>
                    <div className="my-6">
                    ...
                    <div className="mt-6 flex justify-center">
                        <button onClick={addNewplace} className="rounded px-8 py-2 text-white bg-lightblue">Save</button>
                    </div>
                </div>
